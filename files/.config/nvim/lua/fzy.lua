...
local function compute(needle, haystack, D, M, case_sensitive)
  ...
  for i = 1, m do haystack_chars[i] = haystack:sub(i, i) end
...
function fzy.positions(needle, haystack, case_sensitive)
  ...
  elseif n == m then
    local consecutive = {}
    for i = 1, n do consecutive[i] = i end
...
function fzy.get_score_min() return SCORE_MIN end

-- The score returned for exact matches. This is the highest possible score.
function fzy.get_score_max() return SCORE_MAX end

-- The maximum size for which `fzy` will evaluate scores.
function fzy.get_max_length() return MATCH_MAX_LENGTH end
...
function fzy.get_score_floor() return MATCH_MAX_LENGTH * SCORE_GAP_INNER end
